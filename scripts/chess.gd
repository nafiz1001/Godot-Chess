...
func on_square_click(new_square: Vector2i):
	if selected_piece == null or selected_square == null:
		...
		if new_piece != null and new_piece.colour == selected_piece.colour:
			...
			var move_object = valid_moves_2(new_square)
			if move_object != null:
...
func valid_moves_2(from: Vector2i) -> Array[Array]:
	# Generate all valid moves for the piece at the given square
	# considering other pieces on the board, excludes
	# special moves like check, castling, en passant, etc.

	var from_piece = get_piece_at(from)
	if from_piece == null:
		return []
	var all_moves = from_piece.valid_moves_1(from)
	var valid_moves: Array[Array] = []
	for path in all_moves:
		var valid_path: Array[ChessMove] = []
		for _move_object in path:
			var move_object: ChessMove = _move_object
			if move_object.square == from:
				# Skip the move if it leads back to the original square
				continue
			var target_piece = get_piece_at(move_object.square)
			if move_object.capture_only and (target_piece == null or target_piece.colour == from_piece.colour):
				# Capture only move but no enemy piece to capture
				continue
			if move_object.no_capture_only and target_piece != null:
				# No capture move but there's a piece in the way
				continue
			if target_piece != null and target_piece.colour == from_piece.colour:
				# Can't move onto a square occupied by a friendly piece
				break
			valid_path.append(move_object)
			if target_piece != null:
				# Can't move past an enemy piece
				break
			# If all checks passed, add the path to valid moves
		valid_moves.append(valid_path)
